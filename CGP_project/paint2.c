...
void drawSpiral(int, int, int, int);
...
void drawSpiral(int centerX, int centerY, int numTurns, int numPoints) {
    glBegin(GL_LINE_STRIP);

    float angleIncrement = 2.0f * 3.14 / numPoints;
    float radiusIncrement = 1.0f / numPoints;
    float radius = 0.0f;

    for (int i = 0; i <= numTurns * numPoints; ++i) {
    ...
    if (btn == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
    {
        ...
        case(SPIRAL):
            ...
            case(0):
                ...
                break;
            case(1):
                drawSpiral(xp[0], yp[0], abs(x - xp[0]), 100);
                draw_mode = 0;
                count = 0;
                break;
            }
...
int pick(int x, int y)
{
    ...
    else if (x < 7 * ww / 10) return SPIRAL; // Added condition for spiral tool
    else return 0;
}
...
void mydisplay(void)
{
    ...
    screen_box(0, wh - ww / 10, ww / 10);   // line box
    ...
    glColor3f(0.0, 0.0, 0.0);
    screen_box(ww / 2, wh - ww / 10, ww / 10);    // Circle tool box

    glBegin(GL_LINE_LOOP); // Circle symbol in Circle tool box
    for (int i = 0; i < 360; ++i) {
        float angle = i * (3.14 / 180.0);
        float x = ww / 2 + (ww / 40) * cos(angle);
        float y = wh - ww / 20 + (ww / 40) * sin(angle);
        glVertex2f(x + 10, y);
    }
    glEnd();

    glColor3f(0.0, 0.0, 0.0);
    screen_box(3 * ww / 5, wh - ww / 10, ww / 10);    // Spiral tool box

    glBegin(GL_LINE_STRIP); // Spiral symbol in Spiral tool box
    for (int i = 0; i < 360; ++i) {
        float angle = i * (3.14 / 180.0);
        float x = 3 * ww / 5 + (ww / 40) * cos(angle);
        float y = wh - ww / 20 + (ww / 40) * sin(angle);
        glVertex2f(x + 10, y);
    }
    glEnd();
